...
static const char* TRAIN_CSV = "/home/dan/HLS/VivadoProjects/licenta/datasets/training/datatraining_stripped.csv";
static const char* TEST_CSV  = "/home/dan/HLS/VivadoProjects/licenta/datasets/test/datatest_stripped.csv";
...
static void run_frame(hls::stream<rx_byte_axis_t>& rx, hls::stream<anomaly_packet_t>& out,
                      const long* vals, int n, int active, int opcode, int tlast) {
    hls::stream<sensor_packet_t> raw; hls::stream<addr_packet_t> addr;
    send_frame(rx, vals, n, active, opcode, tlast);
    packet_assembler(rx, raw); address_engine(raw, addr); hbos_engine(addr, out);
}
...
int main(){
    ...
    for(int ci=0; ci<N_CONFIGS; ci++){
        ...
        stream_csv(rx,out,TRAIN_CSV,c,OP_TRAIN);
        stream_csv(rx,out,TRAIN_CSV,c,OP_CALIB);
        ...
        stream_csv(rx,out,TEST_CSV,c,OP_DETECT,&tp,&fp,&fn,&tn);
